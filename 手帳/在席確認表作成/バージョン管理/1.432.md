---
notion-id: 26237ce62871801699c4d876ffa0333c
作成日: Invalid date
---
# 概要
...
JSONP を廃止し、fetch(POST)（application/x-www-form-urlencoded）に全面移行。

経路を Browser → Cloudflare Workers → GAS (doPost) に統一。
CORS で Origin を固定、メソッド固定、最小ヘッダでセキュア化。

CSP を厳格化：connect-src は Worker のみ許可（'self' を外す運用推奨）、object-src 'none'、インライン最小化。
※ connect-src は**オリジン指定（末尾 / を付けない）**に統一。

GitHub Pages のキャッシュ影響を回避するため、リリース時は ?v=yyyymmddhhmm を付ける運用を推奨。

認証・権限

3ロール（user / officeAdmin / superAdmin）を運用。

スーパー管理者は **HMAC（salt + password を鍵, nonce をメッセージ）**で追加認証。
getNonce → HMAC 生成 → login(hmac, nonce) → role: superAdmin。

トークンの TTL と自動更新（期限の 80% で renew）を実装。

データモデル・API

設定（名簿）getConfig: {version, updated, groups, menus}。
groups[].members[] は {name, ext, id}。id 不正はクライアントで補正（ハッシュ生成）。

在席データ get / set は 1人単位の差分更新を標準化。

管理 API: listOffices, getFor, getConfigFor, setFor（100件チャンク）, setConfigFor, renameOffice, setOfficePassword, getNonce。

CSV I/O 強化：正規ヘッダ固定＆数百件対応（サーバ側バックアップ前提）。

フロント（UI/UX）

レイアウトを v1.42 値で再構築（列幅変数を復元）。

グリッドは最大3列、1列になったらカード化を強制（board[data-cols="1"] / board.force-cards）。
表⇄カードの切り替えでも操作性を維持。

ステータスは menus.statuses から色/属性を動的生成（requiresTime, clearOn, color）。

requiresTime に応じて戻り時間セレクトを動的 enable/disable、clearOn で値クリア。

ローカル保存キーを v4（officeId + configUpdated）に更新。旧 v3 は初回ログイン時に移行。

擬似ラベルの二重表示を修正（v1.431）：
平常時は擬似ラベルを消し、カード化時は ::before でのみ表示。label.sr-only はカード化時に非表示に。

運用・キャッシュ

Service Worker（sw.js）は名称（CACHE_NAME）を環境で分ける運用を推奨（将来のオフライン拡張に備える）。

Worker は POST 経由のみ、ブラウザキャッシュは no-store 指定。

不具合修正（今回の incident）

admin_super.gs のトップレベル定数の二重定義による GAS 500/HTML 応答 → Worker 502 を解消。
admin_super.gs は 本体と衝突しない定数名（ADMIN_SUPER_PROP_*）に変更。
以後、GAS は**編集後も必ず「既存Webアプリを編集して再デプロイ」**する運用を徹底。

2. 現行仕様（Spec）
アーキテクチャ

Frontend（静的）：GitHub Pages（or 任意ホスティング）

Proxy：Cloudflare Workers（REMOTE_ENDPOINT）

Backend：GAS（Webアプリ doPost）

セキュリティ

CSP：default-src 'self'、connect-src は Worker オリジンのみ。

通信：POST 固定、application/x-www-form-urlencoded。

認証：GAS発行トークン + 自動 renew。
...
superAdmin：HMAC 追加認証で全拠点操作

重要操作（CSV/削除/PW変更）は getNonce→HMAC で再認証。
...

{
  ...
  groups: [{ title: string, members: [{ id: string, name: string, ext?: string }] }],
  menus?: {
    statuses: [{ value: string, color: "\#RRGGBB", requiresTime: boolean, clearOn: boolean }],
    noteOptions: string[],
...
{
  updated: number,
  data: { [memberId: string]: { ext: string, status: string, time: string, note: string } },
  full?: boolean // setFor 初回チャンクで true
}

フロント挙動

起動：publicListOffices → ログイン（super は HMAC→NGなら通常PW）→ getConfig → render → get 適用。

同期：データ 2s ポーリング(+ジッター)、設定 120s 監視、renew は TTL の 80%。

レイアウト：PANEL_MIN_PX=760、MAX_COLS=3、1列で常時カード化。擬似ラベルはカード時のみ。

編集保護：フォーカス/IME中は差し替え抑止。

CSV I/O：フロントでパース → サーバにチャンク適用。

エラーハンドリング

通信失敗・認証失効：logout() → ログイン画面。

トースト通知（成功/失敗色分け）、診断バナー（必要時のみ表示）。

3. 今後の開発計画（Roadmap）
可用性・運用

バックアップ/リストア API（JSON 丸ごと）

Worker 側レート制限/WAF（IP/UA ベース）

監査ログ（誰が・いつ・何を、CSV含む）

使い勝手

オフライン一時保存 + 再接続時自動反映（競合はサーバ勝ち）
...
キーボード操作最適化（矢印移動、Enter 遷移、IME考慮）
...
大規模拠点向け 仮想スクロール／差分パッチ配信

get/set の TTL キャッシュ + If-None-Match

連携

Slack/Teams 通知（特定ステータス遷移時）
