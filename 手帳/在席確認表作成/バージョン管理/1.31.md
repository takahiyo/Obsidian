---
notion-id: 25e37ce6287180c9b727dac5db73f608
作成日: Invalid date
---
# 概要
...
<style>
  ...
  .grp-menu{
    ...
    min-width: 260px; max-width: 90vw; max-height: 60vh; overflow:auto;
...
const REMOTE_POLL_MS  = 2000;        // 反映短縮
...
function render(){
  ...
  recolor();
  startGridObserver();
  buildGroupMenu(); // タイトルのメニューを再構築（総数も更新）
}

/* ===== グループメニュー（タイトル→一覧→スクロール + 総数表示） ===== */
function buildGroupMenu(){
  ...
  if(menuTitle){
    ...
    const h4 = menuEl.querySelector('h4');
    if(h4) h4.textContent = `グループにジャンプ`;
  }

  // 先頭に「全体（合計：XX名）」を置き、その後に各グループ
  const items = [];
  items.push(
    `<li><button class="grp-item" role="menuitem" data-target="top">全体（合計：${total}名）</button></li>`
  );
  ...
  GROUPS.forEach((g,i)=>{
    ...
    items.push(
      `<li><button class="grp-item" role="menuitem" data-target="grp-${i}">
...
/* ==== ★ サーバ値の適用：編集中のセルは上書きしない ==== */
function isEditingField(el){
  if(!el) return false;
  if(el.dataset && el.dataset.editing === '1') return true;      // focus中
  if(el.dataset && el.dataset.composing === '1') return true;     // IME合成中
  if(el === document.activeElement) return true;                  // 念のため
  return false;
}
function setIfNeeded(el, newVal){
  if(!el) return;
  if(isEditingField(el)) return;           // ★編集中は絶対に触らない
...
function unmarkEditing(el){
  if(!el) return;
  delete el.dataset.editing;
}
function wireEvents(){
  // 共通：フォーカス/ブラーで編集中フラグを管理
