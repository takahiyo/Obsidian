---
notion-id: 25e37ce6287180f9b257c53da4f4cfcc
作成日: Invalid date
---
# 概要
...
- Ver1.31に加えて、
	- 多拠点対応
		- 拠点管理者による管理パネル用意
			- 拠点を選択し、管理パスワードでログインすると管理パネルが表示できるようになる
				- CSVインポート・エクスポート
				- 拠点名変更
				- 拠点パスワード変更
	- スーパー管理者
		- スーパー管理者パスワードで管理パネル拡張
	- ログオフ時に背景のパネル表示を非表示へ以降
...
```jsx
/** 在席表 同期API（JSONP） - 役割分離版（固定スーパ管理者PW）
 *  変更点:
 *   - スーパ管理者PWは固定: 'iwCzWDMhY8SU'
 *   - TESTからの自動読取を撤去
 *   - 役割: user / officeAdmin / superAdmin
 *   - 拠点管理者は自拠点のみ操作可、拠点追加/削除はスーパ管理者のみ
 *   - OFFICES_VERSION によるトークン失効は継続
 */
...
const TOKEN_VERSION_PREFIX = 'tver_';
const CACHE_TTL_SEC   = 20;
const KEY_PREFIX      = 'presence:';
const OFFICES_KEY     = 'presence:OFFICES_JSON';
const OFFICES_VERSION_KEY = 'presence:OFFICES_VERSION';
const MAX_SET_BYTES   = 120 * 1024;
const RESET_DATA_ON_CONFIG_CHANGE = false;

/** 固定スーパ管理者PW（ここを変更する場合は再デプロイが必要） */
const MASTER_ADMIN_PASSWORD = 'iwCzWDMhY8SU';

// 既定オフィス: test は表示名を Administrator に変更
const DEFAULT_OFFICES = {
  nagoya: { name: '名古屋中央営業所', password: 'NagoChu6261', adminPassword: 'NagoChu0802' },
  test:   { name: 'Administrator',      password: 'test123',     adminPassword: 'test0802'     },
};
    ...
    for (var k in all) {
      if (k.indexOf(TOKEN_PREFIX) === 0) {
        ...
        if (!exp || exp < t) {
          ...
          prop.deleteProperty(TOKEN_VERSION_PREFIX+ token);
        }
      }
    }
  }
...
function getOfficesVersion_(){
  const p = PropertiesService.getScriptProperties();
  let v = Number(p.getProperty(OFFICES_VERSION_KEY) || 0);
  if (!v) { v = 1; p.setProperty(OFFICES_VERSION_KEY, String(v)); }
  return v;
}
function checkTokenVersion_(prop, token){
  const tv = String(prop.getProperty(TOKEN_VERSION_PREFIX + token) || '');
  const cur= String(getOfficesVersion_());
  return (tv === cur);
}
...
function getOffices_(){
  ...
  if(!v){
    ...
    if(!p.getProperty(OFFICES_VERSION_KEY)) p.setProperty(OFFICES_VERSION_KEY, '1');
  ...
  try{
    ...
    if(changed){
      ...
      const cur = Number(p.getProperty(OFFICES_VERSION_KEY)||1);
      p.setProperty(OFFICES_VERSION_KEY, String(cur+1));
    }
    if(!p.getProperty(OFFICES_VERSION_KEY)) p.setProperty(OFFICES_VERSION_KEY, '1');
    return obj;
  }catch(e){
...
function defaultConfigTest_(){
  return { version:2, updated: now(), groups: [ {title:"テスト1", members:[{name:"test1さん"}]}, {title:"テスト2", members:[{name:"test2さん"}]} ] };
}
...
function doGet(e){
  ...
  if (action==='login'){
    ...
    // スーパ管理者（固定PW）
    if (pw && pw===MASTER_ADMIN_PASSWORD){
      const token = Utilities.getUuid();
      const ver = String(getOfficesVersion_());
      prop.setProperty(TOKEN_PREFIX+token, String(now()+TOKEN_TTL_MS));
      prop.setProperty(TOKEN_OFFICE_PREFIX+token, office); // 表示/既定用
      prop.setProperty(TOKEN_ROLE_PREFIX+token,   'superAdmin');
      prop.setProperty(TOKEN_VERSION_PREFIX+token, ver);
      const name = (offs[office] && offs[office].name) ? offs[office].name : (offs.nagoya && offs.nagoya.name) ? offs.nagoya.name : office;
      return outJSONP(cb,{token, exp:TOKEN_TTL_MS, office, officeName:name, role:'superAdmin'});
    }

    // 拠点の user / officeAdmin
    if (!offs[office]) return outJSONP(cb,{error:'unauthorized'});
    if (!(pw===offs[office].password || pw===offs[office].adminPassword)) {
      return outJSONP(cb,{error:'unauthorized'});
    }
    const role = (pw===offs[office].adminPassword)? 'officeAdmin' : 'user';
    const token = Utilities.getUuid();
    const ver = String(getOfficesVersion_());
    prop.setProperty(TOKEN_PREFIX+token, String(now()+TOKEN_TTL_MS));
    ...
    prop.setProperty(TOKEN_VERSION_PREFIX+token, ver);
    return outJSONP(cb,{token:token, exp:TOKEN_TTL_MS, office:office, officeName:offs[office].name, role});
  }

  if (action==='renew'){
    const token = safeParam_(e,'token','');
    if (!checkToken_(prop,token)) return outJSONP(cb,{error:'unauthorized'});
    if (!checkTokenVersion_(prop, token)) return outJSONP(cb,{error:'unauthorized'});
    prop.setProperty(TOKEN_PREFIX+token, String(now()+TOKEN_TTL_MS));
  ...
  const isAnyAdmin = (isSuper || isOfficeAdmin);
  ...
  // 互換API（adminPw直接指定）: 残すが既定は管理トークン経由推奨
  if (action==='setConfig'){
    const office = tokenOffice;
    const offs = getOffices_();
  ...
  // 拠点一覧: スーパ=全件, 拠点管理者=自拠点のみ
  if (action==='listOffices'){
    const offs = getOffices_();
    const offices = isSuper
      ? Object.keys(offs).map(id=>({id, name: offs[id].name}))
      : [{ id: tokenOffice, name: offs[tokenOffice].name }];
    return outJSONP(cb, {offices});
  }

  // スコープ制約ヘルパ
  function assertOfficeScope_(office){
    if (!office) return {error:'no_office'};
    const offs = getOffices_();
    if(!offs[office]) return {error:'no_office'};
    if (!isSuper && office !== tokenOffice) return {error:'forbidden'};
    return {ok:true};
  }

  if (action==='getFor'){
    const office = safeParam_(e,'office','');
    const scope = assertOfficeScope_(office); if(scope.error) return outJSONP(cb, scope);
    const DATA_KEY = dataKeyForOffice_(office);
    const noCache = safeParam_(e,'nocache','')==='1';
    const cKey    = KEY_PREFIX + 'data:' + office;
    const hit     = noCache ? null : cache.get(cKey);
  ...
  // 拠点追加：スーパのみ
  if (action==='addOffice'){
    if (!isSuper) return outJSONP(cb,{error:'forbidden'});
  ...
  if (action==='setOfficePassword'){
    const id  = safeParam_(e,'id','');
    const scope = assertOfficeScope_(id); if(scope.error) return outJSONP(cb, scope);
    const pw  = safeParam_(e,'password','');
    const apw = safeParam_(e,'adminPassword','');
    const offs = getOffices_();
    if(!offs[id]) return outJSONP(cb,{error:'no_office'});
