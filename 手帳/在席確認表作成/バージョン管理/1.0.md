---
notion-id: 25e37ce628718088bfd3d0eb4c85a835
作成日: Invalid date
---
# 概要
...
- 一旦思いついた仕様は全て実装
	...
	- ログインパスワード
	- 自動同期、変更内容の自動反映
	- メンバー名簿、グループ分け、リスト内の順番をGASに置くことで内容を隠蔽しつつ編集を容易に
	- 最大3列まである程度可変の表、スマホなどだと1列表示
...
<body>
  <header><h1>ビジネスフロント名古屋中央　在席確認表</h1></header>
...
const REMOTE_POLL_MS  = 8000;        // データの定期取得
...
/* ===== レイアウト（列数 3→2→1 の自動調整） =====
   1パネルあたりの必要目安幅（px）と隙間幅から列数を算出し、--cols に反映する。 */
...
/* ===== 名簿（構成）をクライアント整形 =====
   - サーバからの groups:{title, members:[{name, ext?, id?}]} を前提
   - id が未指定なら name+ext から安定IDを生成 */
...
function rowHtml(member){
  ...
  return `
    <tr id="row-${key}" data-key="${key}" data-status="在席">
      ...
      <td class="ext"><label class="sr-only" for="ext-${key}">内線</label>
        <input id="ext-${key}" name="ext" type="text" maxlength="4" inputmode="numeric" pattern="^[0-9]{0,4}$" placeholder="内線" value="${ext}" />
      </td>
      <td class="status"><label class="sr-only" for="status-${key}">ステータス</label>
        <select id="status-${key}" name="status">${STATUSES.map(s=>`<option value="${s}">${s}</option>`).join("")}</select>
      </td>
      <td class="time"><label class="sr-only" for="time-${key}">戻り時間</label>
        <select id="time-${key}" name="time">${timeOptions()}</select>
      </td>
      <td class="note"><input id="note-${key}" name="note" type="text" list="noteOptions" placeholder="備考" /></td>
    </tr>`;
}
function panelHtml(group){
  return `
    <section class="panel">
...
function render(){
  board.innerHTML = GROUPS.map(g=>panelHtml(g)).join('');
  board.style.display = '';
  wireEvents();
...
/* ===== 行内イベント（同期：即時 or デバウンス） =====
   - ステータス: 変更即時送信（在席系へ戻したら時間/備考をクリア）
   - 戻り時間: 変更即時送信
   - 内線: フィールド確定時に送信（change）
   - 備考: IME合成中は待機、入力は900msデバウンス、確定時は即時送信 */
...
/* ===== 起動：最速描画（名簿とデータを並列取得→即レンダ→ローカル適用→サーバ到着で上書き） ===== */
